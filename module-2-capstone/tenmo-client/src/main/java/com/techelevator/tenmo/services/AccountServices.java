...
public class AccountServices {
    ...
    private Transfers[] viewTransferHistory() {
        HttpEntity entity = getEntity();
        Transfers[] transferArray = restTemplate
                .exchange(API_BASE_URL + "users/" + currentUser.getUser().getId() + "/transfers", HttpMethod.GET,
                        getEntity(), Transfers[].class).getBody();

        viewAndSelectTransfers(entity, transferArray);
        return transferArray;
    }

    private void viewAndSelectTransfers(HttpEntity entity, Transfers[] transferArray) {
        ...
        Transfers details = restTemplate
                .exchange(API_BASE_URL + "transfers/" + choiceId, HttpMethod.GET, entity, Transfers.class).getBody();
    ...
    private void approveTransfer(Transfers transfer) {
        ...
        HttpEntity<Transfers> entityComplete = new HttpEntity<>(transfer, header);
        Transfers transferComplete = restTemplate
                .exchange(API_BASE_URL + "new-transfer/approved", HttpMethod.PUT, entityComplete, Transfers.class)
                .getBody();
    }

    private void getTransferTypeStatusDetails(Transfers[] transferArray, String[] transferString, String[] transferType,
